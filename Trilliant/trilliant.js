...
function pars_value()
{
	...
	h = h + dh;
	text = "Radius of conjugation (rs) ";
	ctx.fillStyle = text_color;
	ctx.fillText(text, w1, h);
	text = roundNumber(rs, 3);
	ctx.fillStyle = value_color;
	ctx.fillText(text, w2, h);		
	...
	ctx.fillStyle = text_color;
	ctx.fillText(text, w1, h);		
	text = roundNumber(angle/DEGREE, 3) + "°";
	ctx.fillStyle = value_color;
	ctx.fillText(text, w2, h);
...
function create_num_vertices(enumeration)
{
	...
	for (i = 0; i < n; i++) 
	{
		...
		if  ( (i > 0)&&(i < 4) || (i > 4)&&(i < 8)  || (i > 8)&&(i < 16)  || (i > 16)&&(i < 20) ||  
			  (i > 20)&&(i < 24) ||(i > 24)&&(i < 28) || (i > 28)&&(i < 32) || ( i > 32)&&(i < 36) || 
		      (i > 36)&&(i < 40) || (i > 40)&&(i < 44) || (i > 44)&&(i < 48) ||
			  (i > 48)&&(i < 52) || (i > 52)&&(i < 56) || (i > 56)&&(i < 60) || 
			  (i > 60)&&(i < 64) || (i > 64)&&(i < 68) || (i > 68)&&(i < 72) || 
			...
			else if ( (number > 95) && (number < 108) )
			{
				meshText.add(
					new THREE.Mesh(
						...
						//new THREE.MeshPhongMaterial({color: 0xffffff, emissive: 0x0000ff, side: THREE.DoubleSide, shading: THREE.FlatShading})));
						new THREE.MeshPhongMaterial({color: 0x990000, emissive: 0x990000, side: THREE.DoubleSide, shading: THREE.FlatShading})));
				generateGeometry( meshText, (number - 96).toString() );	
			}
			else 
			{
				meshText.add(
				...
				generateGeometry( meshText, (number - 108).toString() );	
			}			
		}
