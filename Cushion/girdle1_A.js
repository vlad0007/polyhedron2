...
var lw = 1.15;
var rounnd_cir1 = 0.15;
var rounnd_cir2 = 0.110;
var R3 = 0.174;
var ang_2 = 60*DEGREE;
var ang_3 = 60*DEGREE;
var ang_corner = 45*DEGREE;
...
function init_girdle()
{
	...
	points = cir2.Intersection_LineCircle(ln_O3_w);
	if (points == null)
	{
		return null;
	}
	...
	for(i = 13; i < 16; i++)
	{
		...
		if (y > G[1]) // Выше точки пересечения окружностей cir2 и cir3
		{
			...
			points = cir3.IntersectionLineCircle(line4);
			if (points == null)
			{
				return null;
			}
...
function construction()	
{
	...
	line(ctx, O3, girdle[12], 0, 0.6, 0.5, "Black");
	if (O3[0] >= girdle[4][0])
	{
		line(ctx, O3, girdle[4], 0, -0.6, 0.5, "Black");
	}
	else
	{
		line(ctx, O3, girdle[4], 0, 0.6, 0.5, "Black");
	...
	pt = new Point2D(O3[0] + Math.cos(ang_corner),  O3[1] + Math.sin(ang_corner));
	line(ctx, O3, pt, 0, 0.5, 0.5, "Black");
	
	draw_angle_txt(ctx, O3, ang_corner, ang_corner + ang_2, 0.6, "2", "rt", "up", 1, "R");
	draw_angle(ctx, O3, ang_corner, ang_corner + ang_2, 0.6-0.01,  1, "R");
	
	draw_angle_txt(ctx, O3, ang_corner - ang_3, ang_corner, 0.5, "3", "rt", "up", 1, "R");
	draw_angle(ctx, O3, ang_corner - ang_3, ang_corner, 0.5 - 0.01,  1, "R");	
...
function pars_value()
{
	...
	ctx.fillText("     circle 1 - circle 3", 15, 420);
	ctx.fillText("            and ", 15, 440);
	ctx.fillText("     circle 2 - circle 3", 15, 460);
}	
...
// кнопки масштабирования и перемещения
...
// кнопки изменения значений параметров
